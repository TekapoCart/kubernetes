...
kubectl --namespace default create -f "$ROOT"/../manifests/proxy-deployment.yaml

# Waiting for the pod to actually deploy correctly
kubectl --namespace default rollout status --request-timeout="5m" -f "$ROOT"/../manifests/proxy-deployment.yaml